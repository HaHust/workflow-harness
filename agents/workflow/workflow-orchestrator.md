...
4. On Codex, resolve the Agent ID to `Codex Name` and `Codex TOML`, then spawn the custom agent by Codex `name`; do not route through a generic agent with a Markdown spec as the role source.
5. After each worker exits, read its handoff and dispatch the approved reviewer profile.
6. Accept stage output only after reviewer PASS or PASS_WITH_NOTES and required risk gates pass.
7. Route REJECT back to the same worker with iteration increment; after MAX_WORKER_REVIEW_ITERATIONS=2 call F01.
8. Before final gate, require knowledge impact decision and update knowledge when dirty.
